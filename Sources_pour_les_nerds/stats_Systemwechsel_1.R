...
# ---- LIVRE SEUL ----
  ...
  max_page <- max(data$page_number, na.rm = TRUE)
  ...
  png("graphes_export/fig1_vitesse_etude_total.png", width = 800, height = 600)
  plot_with_slope(
    cumulative_hours,
    wpm_clean,
    ...
    data_name = paste0("Livre: Systemwechsel 1 - Fauxhumain1 (p9 - p", max_page, ")"),
    y_tick_step = 3
  )
...
#Stats globales (toute lecture) ---------------------------------------------------------
  ...
  data_global$consultbyhundred<- as.numeric(data_global[[11]])
  data_global$consult_matchingtime<- as.numeric(data_global[[10]])
  data_global$words_per_minute <- with(data_global, total_words / time_minutes)

  max_page_global <- max(data_global$page_number, na.rm = TRUE)
...
# ---- Données globales propres ----
  ...
  consulttime <- data_no_outliers_global$consult_matchingtime[
    !is.na(data_no_outliers_global$consult_matchingtime) & data_no_outliers_global$consult_matchingtime != 0
  ]
  consult_clean_global <- data_no_outliers_global$consultbyhundred[
    !is.na(data_no_outliers_global$consultbyhundred) & data_no_outliers_global$consultbyhundred != "unk"]
  
  cumulative_hours_global <- cumsum(time_clean_global) / 60
  cumulative_consulttime <- (consulttime)/60
  mean_last20_clean_global <- mean(tail(wpm_clean_global, 20), na.rm = TRUE)

# ---- Temps total AVEC outliers (lecture + hors lecture)----
...
# ---- Figure 1 : vitesse selon le temps cumulé ----
  png("graphes_export/fig1_study_speed_merged_total.png", width = 800, height = 600)
  plot_with_slope(
    cumulative_hours_global,
    wpm_clean_global,
    xlab = "Heures additionnées d'étude (total)",
    ylab = "Mots par minute",
    main = "Figure 1. Progression de la vitesse d'étude (total, mpm de pages <180 mots non prises en compte)",
    data_name = paste0("Livres: Aussenseiter+Systemwechsel  - Fauxhumain1 (p9 - p", max_page, ")"),
    y_tick_step = 2
  )
...
# Tracé
png("graphes_export/fig2_consult_total.png", width = 800, height = 600)
plot_with_slope(
  cumulative_consulttime,
  consult_clean_global,
  xlab = "Heures additionnées d'étude (total)",
  ylab = "Mots consultés / 100 mots",
  main = "Figure 3. Mots consultés dans dico / 100 mots (pages <180 mots non prises en compte)",
  data_name = paste0("Livres: Aussenseiter+Systemwechsel  - Fauxhumain1 (p9 - p", max_page_global, ")"),
  y_tick_step = 2
)
dev.off()